...
public class killaura extends Mod {
    public static NumberSetting range = new NumberSetting("Range", 1.00, 16.00, 3.00, 1.00);
    public BooleanSetting criticals = new BooleanSetting("Criticals", false);

    public killaura() {
        super("Gorilla", "CHIMPOUT!", Category.CHIMP);
        addSetting/*s*/(range/*, criticals*/);
    }
        ...
        if (this.isEnabled() && mc.world != null && mc.world.getEntities() != null) {
            ...
            if(targets.size() -1 >= 0) {
                ...
                if (mc.player.getAttackCooldownProgress(0.5f) == 1) {
                    //if (criticals.isEnabled()) mc.player.addVelocity(0, 0.3, 0); mc.player.fallDistance = 0.3f; mc.player.setOnGround(false);
